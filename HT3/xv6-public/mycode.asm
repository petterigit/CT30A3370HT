...
main(int argc, char *argv[]) // OWNCODE
{
  ...
  11:	e8 e4 02 00 00       	call   2fa <getreadcount>

  for(i = 1; i < 2; i++)
    printf(1, "%d%s", readcount, i+1 < 2 ? " " : "\n");
  16:	68 e0 06 00 00       	push   $0x6e0
  1b:	50                   	push   %eax
  1c:	68 e2 06 00 00       	push   $0x6e2
  21:	6a 01                	push   $0x1
  23:	e8 98 03 00 00       	call   3c0 <printf>
  exit();
...
putc(int fd, char c)
{
 ...
 4d8:	89 45 d0             	mov    %eax,-0x30(%ebp)
        if(s == 0)
          s = "(null)";
 4db:	b8 e7 06 00 00       	mov    $0x6e7,%eax
...
malloc(uint nbytes)
{
 ...
 65e:	53                   	push   %ebx
 65f:	e8 9e fc ff ff       	call   302 <sbrk>
  if(p == (char*)-1)
